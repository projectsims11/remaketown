...
function formatLog(logJson)
    local formattedLog = ""
    local logTable = json.decode(logJson)
    
    for i, entry in ipairs(logTable) do
        formattedLog = formattedLog .. string.format([[
{ชื่อ %q, จำนวน %d, เป็นเงิน %d}
]], entry.item, entry.count, entry.money)
    end
