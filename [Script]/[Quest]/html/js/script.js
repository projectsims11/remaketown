    ...
    window.addEventListener("message", function (event) {
      ...
      document.onkeyup = function (data) {
        if (data.which == 27) {
          ...
          $.post("https://Quest/Exit", JSON.stringify({}));
          return;
        }
      };
    });
...
const QuestAction = (key) => {
  ...
  if (cls === "btn-quest-active") {
    ...
    $.post("https://Quest/QuestAction", JSON.stringify({key: key, type: "AcceptQuest"}));
  }else {
    $(`#btn-key-${key}`).text(`รับเควส`)
    $(`#btn-key-${key}`).toggleClass("btn-quest-warning btn-quest-active");
    $.post("https://Quest/QuestAction", JSON.stringify({key: key, type: "DropQuest"}));
  }
}
