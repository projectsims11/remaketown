...
RegisterNUICallback('checkBank', function()
	TriggerServerEvent('crew-phone:check-bank',GetPlayerServerId(PlayerId()))
end)

RegisterNetEvent('crew-phone:check-bank')
AddEventHandler('crew-phone:check-bank',function(c)
	SendNUIMessage({event = 'updateBankHistory', history = c})
end)
