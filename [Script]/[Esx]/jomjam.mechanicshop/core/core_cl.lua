...
getPlayerCash = function()
    local data = ESX.GetPlayerData().accounts
                        ...
                        for i = 1, #Config["Positions"], 1 do
                            ...
                            if (tempDist <= tempActionDist) then
                                ...
                                if (not IsHudHidden()) then
                                    ESX.ShowFloatingHelpNotification(Config["Keys"].action.TextHelper, vector3(tempPos.pos.x, tempPos.pos.y, tempPos.pos.z))
                                end

                                if IsControlJustReleased(0, Config["Keys"].action.key) then
                                    if Config["UseJob"] then
                                        if ESX.GetPlayerData().job.name == tempPos.NeedJobName then
...
RegisterNetEvent('jomjam:OpenShopx')
AddEventHandler('jomjam:OpenShopx', function()
	openUI()
end)
...
function updateMenu(menuId)
    if (menuId == nil or Config["Menus"][menuId] == nil) then
        return
    end
                ...
                elseif (data.user == "enter") then
                    ...
                    if (menuOption and menuOption.price) then
                        ...
                        elseif (menuOption.price > 0) then
                            ...
                            -- if
                            --     (customConfigPosIndex and customConfigPosIndex > 0 and
                            --         customConfigPosIndex <= #Config["Positions"] and
                            --         Config["Positions"][customConfigPosIndex])
                            --  then
                                tempPrice = tempPrice
                            -- end

                            if (ESX and (getPlayerCash() >= tempPrice)) then
                                canBuyMod = true

                                if (tempPrice > 0) then
                                    TriggerServerEvent("replayx.mechanicshop:Removemoney", tempPrice)
                                    if Config["Finish"] then
                                        Finish()
                                    end
                                end
...
function optionsShouldShow(menu)
    ...
    for i = 1, #menu.options, 1 do
        ...
        if
            (customConfigPosIndex and customConfigPosIndex > 0 and customConfigPosIndex <= #Config["Positions"] and
                Config["Positions"][customConfigPosIndex])
         then
...
function createMenu(menuId, menuOption)
    ...
    for i = startIndex, GetNumVehicleModData(customVehicle, menuOption.modType), 1 do
        ...
        table.insert(
            Config["Menus"][newMenuId].options,
            {
                label = tempLabel,
                img = menuOption.img,
                price = tempPrice,
