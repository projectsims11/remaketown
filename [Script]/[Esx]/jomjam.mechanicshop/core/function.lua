...
function repairtVehicle(vehicle)
		...
		local t = ((targetEngineHealth - curEngineHealth) + (targetBodyHealth - curBodyHealth)) * Config['Repair']['Time']

		if Config['Repair']['Vehiclerepair'] then
			exports[Config['Loading']['Type']]:Progress({
				duration = t,
				useWhileDead = false,
				canCancel = true,
				controlDisables = {
				...
				label = Config['Loading']['Settings']["Repair"]['Text']
			}, function(status)
				if (not status) then
					-- if Config['Repair']['Repair'] then
					-- 	SetVehicleFixed(vehicle)
					-- end
					SetVehicleDirtLevel(vehicle, Config['Repair']['Cleanliness'])
					updateMenu('main')
					SetVehicleFuelLevel(vehicle, Config['Repair']['Fuel'])
					if Config['Finish_Custom'] then
						Finish_Custom()
					end
				else
					updateMenu('main')
			...
			exports[Config['Loading']['Type']]:Progress({
				duration = Config['Loading']['Settings']["Repair"]['Time'],
				useWhileDead = false,
				canCancel = true,
				controlDisables = {
					disableMovement = true,
