...
$(window).ready(function() {
    ...
    $.post('http://jomjam.mechanicshop/uiReady', {});
});
...
function listener() {
    window.addEventListener('message', (event) => {
        ...
        if (tempData.type === 'open') {
            ...
            } else if (tempData.what === 'money') {
                if (currentMoney != tempData.money) {
                    currentMoney = tempData.money;
                    $('#money').html('เงิน : ' + GetNumberWithCommas(currentMoney));
                    ...
                    $('.options_container_option_price').each(function(index) {
                        ...
                        if (tempPrice) {
                            ...
                            if (tempPrice > currentMoney) {
                                $(this).addClass('money_not');
                            } else {
                                $(this).removeClass('money_not');
                            }
                        }
                    });
                    ...
                    if ($('#colorPicker-holder').css("opacity") >= 0.1) {
                        ...
                        tempPrice = tempPrice * 2;
                        if (tempPrice > currentMoney) {
                            $('#colorPicker-price').addClass('money_not');
                        } else {
                            $('#colorPicker-price').removeClass('money_not');
                        }
                    }
                }
...
document.onkeydown = function(event) {
    if (event.which == 9) {
        ...
        $.post('http://jomjam.mechanicshop/handle', JSON.stringify({
            type: 'update',
            what: 'menu',
            user: 'backspace',
...
function createMenu(menuId, data, title, defaultOption, whitelistJobName) {
    ...
    $('#options_container').stop(true, true).animate({height: '0px'}, 300, function() {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (data[i].price >= 0) {
                let tempClass = '';
                if ((!isOpenByAdmin) && data[i].price > currentMoney) tempClass = 'money_not';
