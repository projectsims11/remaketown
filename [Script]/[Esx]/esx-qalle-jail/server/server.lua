	...
	if xPlayer["job"]["name"] == "police" then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			if jailTime ~= nil then
				...
				TriggerClientEvent('okokNotify:Alert', src, "แจ้งเตือน!", "คุณถูกจำคุกเป็นเวลา " .. jailTime .. " นาที", 5000, 'warning')			
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(firstname, Lastname)
						--TriggerClientEvent('chat:addMessage', -1, { args = { "JUDGE",  name .. " " .. Lastname .. " Is now in jail for the reason: " .. args[3] }, color = { 249, 166, 0 } })						
					end)
				end
			else
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "ศาลพิพากษา [คุกตู้ปลา]","ให้ ".. firstname .. " ติดคุก ในคดี: " .. jailReason }, color = { 249, 166, 0 } })
	end)
	--TriggerClientEvent("esx:showNotification", src, GetPlayerName(targetSrc) .. " Jailed for " .. jailTime .. " minutes!")
end)

RegisterServerEvent("esx-qalle-jail:jailPlayer2")
AddEventHandler("esx-qalle-jail:jailPlayer2", function(targetSrc, jailTime)
	local src = source
	local targetSrc = tonumber(targetSrc)
...
ESX.RegisterServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(source, cb)	
	...
	MySQL.Async.fetchAll("SELECT firstname, lastname, jail, identifier FROM users WHERE jail > @jail", { ["@jail"] = 0 }, function(result)
		for i = 1, #result, 1 do
			table.insert(jailedPersons, { firstname = result[i].firstname .. " " .. result[i].lastname, jailTime = result[i].jail, identifier = result[i].identifier })
		end
		cb(jailedPersons)
	end)
end)
...
RegisterServerEvent('esx-qalle-jail:removemoney')
AddEventHandler('esx-qalle-jail:removemoney', function()
