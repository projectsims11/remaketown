	...
	if distanceCheck <= Config.LoseConnectionDistance then
		...
		if distanceCheck <= 1.5 then
			...
			if not Attached then
				ShowToolTip("~w~[E] เก็บ ~g~[G] ขึ้น ", vector3(GetEntityCoords(RCCar.Entity).x, GetEntityCoords(RCCar.Entity).y, GetEntityCoords(RCCar.Entity).z))
			end
		end
	end
...
RCCar.Spawn = function()
	RCCar.LoadModels({ GetHashKey("rcbandito"), 68070371, GetHashKey(SkateboardModel), "pickup_object", "move_strafe@stealth"})
	...
	AttachEntityToEntity(RCCar.Skate, RCCar.Entity, GetPedBoneIndex(PlayerPedId(), 28422), 0.0, 0.0, -0.15, 0.0, 90.0, 0.0, true, true, true, true, 1, true)
...
ShowToolTip = function (msg, coords)
	AddTextEntry('skateboard', "<font face='sarabun'> ".. msg .. " </font>")
	SetFloatingHelpTextWorldPosition(1, coords)
	SetFloatingHelpTextStyle(1, 1, 2, -1, 3, 0)
	BeginTextCommandDisplayHelp('skateboard')
