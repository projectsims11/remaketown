...
RegisterNetEvent('curfew:start')
AddEventHandler('curfew:start', function()
	if kerfil == false then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'curfew_Police',
		{
			...
			if result == "nil" then
				...
				menu.close()
				TriggerServerEvent('curfew_Police:send',result)
			end
...
function YesorNo()
        ...
        function (data, menu)
          if data.current.value == 'yes' then   
			TriggerServerEvent('curfew_Police:send', '')
			menu.close()			
          end
...
AddEventHandler('redzone:stopredzone', function()
	...
	if location[0] ~= nil then
		...
		TriggerEvent('chatMessage', "ตำรวจ ", {255, 0, 0}, "^8ประกาศยกเลิกเคอร์ฟิว ^3"..results)
...
AddEventHandler('redzone:createForAll', function(x2,y2,z2,result)
	...
	if tostring(GetStreetNameFromHashKey(var2)) == "" then
		textStreet = "<span style='color:OrangeRed'>*โซนอันตราย*</span> <br><span style='color:Red'>ประกาศเคอร์ฟิว </span><span style='color:Gold'>"..result.."</span><br>โดย: <span style='color:OrangeRed'>[เจ้าหน้าที่ตำรวจ]</span><br><span style='color:White'>ผู้ไม่เกี่ยวข้องออกจากพื้นที่ใน 30 วินาที</span>"
		TriggerEvent('chatMessage', "ตำรวจ ", {255, 0, 0}, "^8ประกาศเคอร์ฟิว ^3 "..result.." ^8ขอให้ประชาชนที่ไม่เกี่ยวข้องออกจากพื้นที่ด้วยครับ!!")
