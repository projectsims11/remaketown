...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		if data.current.value == 'jail_menu' then
			TriggerEvent("Fewthz_Jail:openMenu")
...
function CreateBillingPrice(player, value)
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'dialog_createbill_price',
	{
		...
		if tonumber(data2.value) then
			TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'police',"Fine: "..value, tonumber(data2.value))  --society_police
			menu2.close()
			TriggerEvent("pNotify:SendNotification", {
...
function ImpoundVehiclePolice(vehicle)
	...
	ESX.Game.DeleteVehicle(vehicle) 
	TriggerEvent("pNotify:SendNotification", {
		text = '<strong class="green-text">ยึดรถเรียบร้อยแล้ว</strong>',
		type = "success",
		timeout = 3000,
		layout = "bottomCenter",
		queue = "global"
...
RegisterKeyMapping('policeMenu', 'Open Police Menu', 'keyboard', 'F6')