...
local cruisespeed = 110 -- Default autodrive speed. You can change the speed with (num +) and (num -) 
local display = false
local xpos = 0
local ypos = 0 
                ...
                if Config.EnableAutodrive then
                    --[[if IsControlJustPressed(1, Config.AutodriveKeycode) and autodrive == false then
                        if IsWaypointActive() then                              
                            ...
                            --TaskVehicleDriveToCoordLongrange(xPlayer, veh, x, y, ground, 10.0, 786603, 20.0)
                            --SetDriverAbility(PlayerPedId(), 100.0)
                            SetDriverAbility(xPlayer, 1.0)        -- values between 0.0 and 1.0 are allowed.
                            SetDriverAggressiveness(xPlayer, 0.0) -- values between 0.0 and 1.0 are allowed.
                            TaskVehicleDriveToCoordLongrange(xPlayer, veh, x, y, ground, 20.0, 786492 , 20.0)
                            SendNotification({
                    ...
                    if autodrive then
                        local veh = GetVehiclePedIsIn(PlayerPedId(), false)  
                        local pos = GetEntityCoords(veh)     
                        SetDriveTaskCruiseSpeed(PlayerPedId(),124.0)
...
RegisterKeyMapping('openpilot', 'Open Pilot', 'keyboard', 'G')

RegisterCommand('openpilot', function()
    local playerPed = PlayerPedId()
    if IsPedSittingInAnyVehicle(playerPed) then
        if not isInPilotUI then
            SendNUIMessage({
                action="OpenPilot"
            })
...
RegisterNUICallback("ClosePL",function()
    if isInPilotUI then
        SendNUIMessage({
            action="ClosePilot"
        })
...
RegisterNUICallback("pilot",function()
    if autodrive == false then
        if IsWaypointActive() then       
            local xPlayer = PlayerPedId()                       
            local WaypointHandle = GetFirstBlipInfoId(8)                    
            x,y,z = table.unpack(Citizen.InvokeNative(0xFA7C7F0AADF25D09, WaypointHandle, Citizen.ResultAsVector())) 
...
RegisterNUICallback("stoppilot",function()
    if autodrive then
        ClearPedTasks(PlayerPedId())
        SendNotification({
            text = "Autopilot disabled.",
...
RegisterNUICallback("speedup",function()
    if autodrive then
        if cruisespeed < Config.AutodriveMaxSpeed then
            cruisespeed = cruisespeed + 2.5
        else
...
RegisterNUICallback("speeddown",function()
    if autodrive then
        if cruisespeed > Config.AutodriveMinSpeed then
            cruisespeed = cruisespeed - 2.5
        else
