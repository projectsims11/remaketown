            ...
            --        if (not tempPos.whitelistJobName or jobName == tempPos.whitelistJobName) then
                        ...
                        if (tempDist <= tempActionDist) then
                            ...
                            if (not IsHudHidden()) then
                                ...
                                exports["Fewthz_TextUI"]:ShowHelpNotification('Press ~' .. Config.Keys.action.name .. '~ to enter customs')
                            end
...
function closeUI(sendToUI, resetVehToDefault)
    ...
    SetVehicleBodyHealth(vehicle, vehicle_damage)
    uiOpen = false
end
                    ...
                    elseif (menuOption.price > 0) then
                        ...
                        if (ESX and (ESX.GetPlayerData().money >= tempPrice)) then
                            ...
                            if (tempPrice > 0) then 
                                ...
                                local vehDisplayName = GetDisplayNameFromVehicleModel(vehicleModel)
                                local sendToDiscord = '' .. GetPlayerName(PlayerId()) .. ' แต่งรถ ' .. GetLabelText(vehDisplayName) .. ' ทะเบียน ' .. GetVehicleNumberPlateText(customVehicle) .. ' และ เสียค่าใช้จ่าย $' .. ESX.Math.GroupDigits(tempPrice) .. ''
	                            TriggerServerEvent('azael_dc-serverlogs:sendToDiscord', 'LsCustom', sendToDiscord, GetPlayerServerId(PlayerId()), '^2')
                            end
                        else
                            canBuyMod = false
                        end
