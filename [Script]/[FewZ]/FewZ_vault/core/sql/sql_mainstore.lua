...
MySQL.ready(function()
	local result = MySQL.Sync.fetchAll('SELECT * FROM fewz_inventory')
	...
	for i=1, #result, 1 do
		...
		local result2 = MySQL.Sync.fetchAll('SELECT * FROM fewz_inventory_items WHERE name = @name', {
			['@name'] = name
		})
		...
		if shared == 0 then
			...
			if #result2 == 0 then
				MySQL.Sync.execute('INSERT INTO fewz_inventory_items (name, owner, data) VALUES (@name, NULL, \'{}\')', {
					['@name'] = name
				})
...
AddEventHandler('FewZ_vault:getInventory', function(name, owner, cb)
	cb(GetDataStore(name, owner))
end)

AddEventHandler('FewZ_vault:getInventoryOwners', function(name, cb)
	cb(GetDataStoreOwners(name))
end)

AddEventHandler('FewZ_vault:getSharedInventory', function(name, cb)
...
AddEventHandler('esx:playerLoaded', function(source)
	...
	for i=1, #DataStoresIndex, 1 do
		...
		if dataStore == nil then
			MySQL.Async.execute('INSERT INTO fewz_inventory_items (name, owner, data) VALUES (@name, @owner, @data)',
			{
				['@name']  = name,
