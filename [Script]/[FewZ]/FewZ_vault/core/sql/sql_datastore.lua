...
function CreateDataStore(name, owner, data)
	...
	self.save = function()
		...
		local timeoutCallback = ESX.SetTimeout(2000, function()
			if self.owner == nil then
				MySQL.Async.execute('UPDATE fewz_inventory_items SET data = @data WHERE name = @name',
				{
					['@data'] = json.encode(self.data),
					['@name'] = self.name,
				})
			else
				MySQL.Async.execute('UPDATE fewz_inventory_items SET data = @data WHERE name = @name and owner = @owner',
				{
					['@data']  = json.encode(self.data),
					['@name']  = self.name,
					['@owner'] = self.owner,
