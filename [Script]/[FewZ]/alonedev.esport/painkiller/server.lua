...
for i , v in pairs(Config['Painkiller']) do 
	ESX.RegisterUsableItem(v['item_name'], function(source)
		...
		TriggerClientEvent('alonedev.esport:onPainkiller', _source , v )
	end)
end

RegisterNetEvent('alonedev.esport:deleteitem')
AddEventHandler( 'alonedev.esport:deleteitem', function(item)
	local _source = source
	local xPlayer  = ESX.GetPlayerFromId(source)
