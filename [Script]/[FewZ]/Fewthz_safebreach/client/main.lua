...
AddEventHandler("esx:playerLoaded", function()
	...
	ESX.TriggerServerCallback("Fewthz_safe:checkSafeBreaches", function(successfull)
		if successfull then
...
RegisterNetEvent("Fewthz_safe:eventHandler")
AddEventHandler("Fewthz_safe:eventHandler", function(event, eventData)
		...
		if DoesEntityExist(closestSafe) then
			...
			if dstCheck <= 2.5 then 
				...
				if display then
					...
					local markerCoords = GetOffsetFromEntityInWorldCoords(closestSafe, -0.5, -0.3, -1.2 + 0.5)
