...
function ManageFuelUsage(vehicle)
	if not DecorExistOn(vehicle, Config.FuelDecor) then
		...
		--ESX.TriggerServerCallback('nc:getfuel',function(fuel)
		--end, plate)
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		local fuelToAdd = 0.05
		-- local extraCost = fuelToAdd * 5
		-- local fuelToAdd = math.random(50, 60) / 15.0
		-- local extraCost = fuelToAdd + 0.3 -- เดิม
		local extraCost = fuelToAdd * Config.Price -- แก้
	...
	if pumpObject then
		...
		TriggerServerEvent('esx:updateWeaponAmmo','WEAPON_PETROLCAN', tankFuel)
	end
...
-- local WorldFuelClose = false
-- local i = 0
-- RegisterNetEvent('fuel:world')
-- AddEventHandler('fuel:world', function(x , y)
-- 	if y == 'add' then
-- 		i = i + x
-- 		print('New i = '..i)
-- 	else
							...
							if currentCash > 0 then
								...
								if IsControlJustReleased(0, 38) then
									if isbusy then
										...
										exports['mythic_notify']:DoHudText('error', 'ใจเย็นๆกันหน่อยวัยรุ่น');
									end
								end
							else
...
function CreateBlip(coords)
	...
	AddTextComponentString('<font face="sarabun">ปั้มน้ำมัน</font>')
...
if Config.EnableHUD then
	...
	Citizen.CreateThread(function()
		while true do
			...
			--[[if tonumber(kmh) > 20 then
				DisablePlayerFiring(PlayerPedId(), true)
			end]]

			if displayHud then
				if tonumber(fuel) < 30 then
					DrawAdvancedText(0.314 - x, 0.90 - y, 0.015, 0.0028, 0.6, fuel, 229, 115, 115, 255, 6, 1)
				else
					DrawAdvancedText(0.314 - x, 0.90 - y, 0.015, 0.0028, 0.6, fuel, 255, 255, 255, 255, 6, 1)
				end
		
				DrawAdvancedText(0.270 - x, 0.90 - y, 0.015, 0.0028, 0.6, kmh, 100, 181, 246, 255, 6, 1)
				
				--DrawAdvancedText(0.148 - x, 0.7765 - y, 0.005, 0.0028, 0.4, "mp/h              km/h              Fuel", 255, 255, 255, 255, 6, 1)
				DrawAdvancedText(0.288 - x, 0.9065 - y, 0.015, 0.0028, 0.4, "km/h              Fuel", 255, 255, 255, 255, 6, 1)
			else
				Citizen.Wait(750)
			end
		end
