...
RegisEvent(true, events.cl["OnOpenInventoryMobile"], function(veh)
	local t = PlayerPedId()
	if not IsEntityDead(t) then
		_C_Veh_Last = xZero.C.Veh(veh)
		if not _C_Veh_Last.IsVaild() then
			return nil
		end
...
xZero.Funcs.Vehicle_Vaild = function(J)
	...
	if N > Config.Vehicle_Distance then
		NotifyT("vehicle_distance_error", {
			J.plate
		}, "error")
		return false
	end;
