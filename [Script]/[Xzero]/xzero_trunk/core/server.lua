...
--[[
	...
	local e = "http://xzero.in.th/xFiveM/xzero_scripts/xzero_trunk.php?x="..math.random(10000000, 99999999)
	local f = 0;
	b = function()
...
EventsInit = function()
	RSC(events.sv["OnInventoryDataRequestCB"], function(l, m, n, o)
		...
		xZero.Funcs.OwnerVehicles_GetByPlate(function(r)
			...
			if Config.Vehicle_OwnerOnly and s and r ~= q then
				local t = VJob(p.getJob().name)
				if not(t.Vaild and t.OwnerOnly_Allow()) then
					NotifyT(l, "vehicle_owneronly_error", {
						n.plate
					}, "error")
					m(false)
					return
	...
	RegisEvent(true, events.sv["OnPut"], function(w, x)
		...
		if y.weight_max ~= -1 and D > y.weight_max then
			NotifyT(l, "weight_error", nil, "error")
			u.Prograss = false;
			return
		end;
		local F = false;
