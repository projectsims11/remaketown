...
if GetConvar('voice_useNativeAudio', 'false') == 'true' then
	...
	Cfg.voiceModes = {
		...
		{3.0, "🔉 ปกติ"}, -- Normal speech distance in gta distance units
		{6.0, "🔊 ตะโกน"} -- Shout speech distance in gta distance units
	}
else
	Cfg.voiceModes = {
		{3.0, "🔈 กระซิบ"}, -- Whisper speech distance in gta distance units 
		{7.0, "🔉 ปกติ"}, -- Normal speech distance in gta distance units
		{15.0, "🔊 ตะโกน"} -- Shout speech distance in gta distance units
	}
end
