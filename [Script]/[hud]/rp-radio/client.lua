...
AddEventHandler('esx:setJob', function(job)
    ...
    if ESX.PlayerData.job and (ESX.PlayerData.job.name == 'ambulance' or ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'mechanic') then 
        exports["rp-radio"]:GivePlayerAccessToFrequencies(7)
    else
        exports["rp-radio"]:RemovePlayerAccessToFrequencies(7)
    end  
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        -- Open radio settings
        if isActivatorPressed and isSecondaryPressed and not isFalling and Radio.Enabled and Radio.Has and not isDead and checkHasItem('radio') then
            Radio:Toggle(not Radio.Open)
        elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
            Radio:Remove()
            -- exports["mumble-voip"]:SetMumbleProperty("radioEnabled", false)
