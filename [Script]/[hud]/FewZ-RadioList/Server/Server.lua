...
AddEventHandler("playerDropped", function()
	...
	for _, player in pairs(playersInCurrentRadioChannel) do
		if player.Source ~= src then
			TriggerClientEvent("JLRP-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, 0,
...
function Connect(src, currentRadioChannel, radioChannelToJoin)
	...
	Wait(500) -- Wait for pma-voice to finilize setting the player radio channel

	local playersInCurrentRadioChannel = CreateFullRadioListOfChannel(radioChannelToJoin)
	for _, player in pairs(playersInCurrentRadioChannel) do
		TriggerClientEvent("JLRP-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, radioChannelToJoin,
			playersInCurrentRadioChannel)
	end
	playersInCurrentRadioChannel = {}
end
...
RegisterNetEvent('FewZ-RadioList:SV:RemoveAllUI')
AddEventHandler('FewZ-RadioList:SV:RemoveAllUI', function()
    local src = source
    TriggerClientEvent("RemoveAllById:RadioList", -1, src, "remove", "", "")
end)
RegisterNetEvent('FewZ-RadioList:SV:AddAllUI')
AddEventHandler('FewZ-RadioList:SV:AddAllUI', function(mhz)
    local src = source
    local xPlayer = Core.GetPlayerFromId(src)
    TriggerClientEvent("RemoveAllById:RadioList", -1, src, "add", xPlayer.getName(), mhz)
end)