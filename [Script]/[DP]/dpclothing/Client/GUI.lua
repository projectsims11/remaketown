...
function SoundPlay(which)
	...
	if IsPedInAnyVehicle(ped) and not Config.GUI.AllowInCars then return end
...
local function DrawGUI()
	...
	if Cooldown then Text(x, y-0.06, 0.28, Lang("PleaseWait"), false, false, true) end 		-- Cooldown indicator, if theres a cooldown we display a little text.
...
RegisterNetEvent('dpclothing:change_state_show')
AddEventHandler('dpclothing:change_state_show', function(state)
    stateShow = state
end)


RegisterNetEvent('dpclothing:close')
AddEventHandler('dpclothing:close', function()
    print('close menu open')
    MenuOpened = false
end)

RegisterKeyMapping('dpClothing', 'DP Clothing', 'keyboard', 'M')
