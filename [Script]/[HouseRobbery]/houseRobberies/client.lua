...
local robbableItems = Config.robbableItems
local PlayerData = {}
...
--           if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), v.x, v.y, v.z, false) < 2.0 then
--               inzone = true
--               break
--           else
--               inzone = false
...
AddEventHandler("houseRobberies:attempt", function(lockpicks)
  ...
  if inzone then
            ...
            if result then
                ...
                for id, v in pairs(robbableHouses) do
                    if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
                        ...
                        exports["progressBars"]:startUI(12000, "กำลังงัดประตู")
                        Wait(12000)
                        isLockpicking = false
                        pedSpawned = false
        ...
        elseif lockpicks == 0 and not isRobbing then
            exports["okokNotify"]:Alert("แจ้งเตือน", "คุณไม่มีที่งัด", 4000, "error")
        elseif not isNight() and not isRobbing then
            exports["okokNotify"]:Alert("แจ้งเตือน", "มันสว่างเกินไป", 4000, "error")
        end
    else
      TriggerEvent("pNotify:SendNotification", {
        text = 'ไม่สามารถใช้ข้างนอกได้',
        type = "error",
        timeout = 3000,
        layout = "bottomCenter",
        queue = "global"
  ...
  if isRobbing then
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
      sleep = 0
      DrawText3Ds(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], '~w~Press ~g~G~s~ To Searched ' .. myRobbableItems[i]["name"])
     ...
     if IsControlJustReleased(1, 47) then
      ...
      TriggerEvent("mythic_progbar:client:progress", {
        ...
        duration = 20000,
        label = "กำลังค้นหา "..myRobbableItems[i]['name'],
        useWhileDead = false,
        canCancel = false,
        controlDisables = {
...
function agroNPC()
 ...
 repeat
  local pos = GetEntityCoords(ped)
  local distance = GetDistanceBetweenCoords(playerCoords, pos, true)
  if distance < 50.0 and ped ~= GetPlayerPed(-1) and not IsPedAPlayer(ped) then
   ClearPedTasksImmediately(ped)
   SetPedDropsWeaponsWhenDead(ped,Config.DropsWeaponsWhenDead)
   GiveWeaponToPed(ped, GetHashKey(Config.WeaponNPC), 150, 0, 1)
   SetPedCombatRange(ped, 2)
	 SetPedCombatMovement(ped, 46)
